...
module.exports = {
  ...
  configureWebpack: {// qiankun，在一些低端机上，当qiankun无法从entry export上面拿到微应用生命周期时会尝试从window对象上获取，默认是取window[appName]，当微应用导出的library name和appName不一致时，需要手动指定，否则可能导致浏览器的后退前进按钮出错
    output: {
      library: `${name}-[name]`,
